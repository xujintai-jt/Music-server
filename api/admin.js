...
router.post(
  ...
  (req, res) => {
    let flag = false;
    Music.findOne({ songName: req.body.songName }).then((hasOne) => {
      ...
      if (hasOne) {
        ...
        if (flag) {
          res
            .status(422)
            .json({ status: "422", result: "歌曲存在,请勿重复添加~" });
        } else {
          const song = {
          ...
          newSong
            ...
            .catch((err) => {
              ...
              res
                .status(500)
                .json({ status: "500", result: "未知错误,添加失败" });
            });
        }
      } else {
        const song = {
...
router.post(
  ...
  async (req, res) => {
    const _id = req.body._id;
    await Music.findOneAndRemove({ _id }).then((musics) => {
      musics.save().then(() => {
        console.log("歌曲删除成功");
        res.json({ status: "200", result: "删除成功" });
        delNoUse.delAll();
        AdminLike.findOneAndRemove({ s_id: _id }).then((like) => {
          if (like) {
            like.save().then(() => {
...
router.post(
  ...
  async (req, res) => {
    ...
    await AdminLike.findOne({ s_id }).then((song) => {
      if (song) {
        return res
          .status(416)
          .json({ status: "416", result: "歌曲已收藏,请勿重复收藏" });
      } else {
  ...
  await Music.find().then((allsongs) => {
    AdminLike.find().then((adminlikes) => {
      ...
      allsongs.forEach((item) => {
        ...
        adminlikes.forEach((al) => {
          if (item._id == al.s_id) {
            item.songName.toLowerCase().indexOf(songName) == -1
              ? (flag = true)
              : result.push(item);
            if (flag) {
...
router.get(
  ...
  passport.authenticate("jwt", { session: false }),
  async (req, res) => {
    UserAndOrders.find().then((orders) => {
      res.send(orders);
    });
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    await UserAndOrders.findOneAndRemove({ order_id }).then((orders) => {
      orders.save().then(() => {
        console.log("订单删除成功");
        res.json({ status: "200", result: "删除成功" });
      });
...
router.post("/account/login", (req, res) => {
  ...
  Admin.findOne({ email }).then((admin) => {
    if (!admin) {
      return res
        .status(406)
        .json({ status: "406", result: "用户名或密码错误" });
    } else {
      const password = req.body.password;
...
router.post(
  ...
  async (req, res) => {
    const email = req.body.email;
    await Admin.findOne({ email }).then((islegal) => {
      if (!islegal) {
        ...
        if (!isValidPassword) {
          return res
            .status(416)
            .json({ status: "416", result: "用户名或密码错误" });
        } else {
          res.send("true");
        }
      }
